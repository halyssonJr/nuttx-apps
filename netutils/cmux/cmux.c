  ...
  while (cmux_decode_frame(cmux_handle->stream, parse) >= 0)
  {
    ...
    if (CMUX_FRAME_TYPE(FRAME_TYPE_UI, parse) || CMUX_FRAME_TYPE(FRAME_TYPE_UIH, parse))
    {
      if (parse->address > 0)
      {
        ...
        {
          printf("Frame length less than expected\n");
          continue;
        }
      }
      else
    ...
    else
    {
      ...
      {
      case FRAME_TYPE_UA:
         printf("FRAME TYPE UA \n");

        break;
      case FRAME_TYPE_DM:
        printf("FRAME_TYPE_DM \n");
      ...
      case FRAME_TYPE_DISC:
        ...
        if (ret < 0)
        {
          printf("Failed to encode the frame. Address (%d) \n", parse->address);
        }
        break;
      case FRAME_TYPE_SABM:
        printf("FRAME_TYPE_SABM \n");
        ...
        {
          printf("SABM even though channel %d was already closed.\n", parse->address);
        }
...
int cmux_create(struct cmux_handle_s *cmux_handle, FAR const char * script, char *tty_name, int total_channels)
{
  ...
  if (ret < 0)
  {
    printf("Failed to run cmux script\n");
    goto exit;
  }
  ...
  {
    printf("Failed to allocate memory to channels\n");
    goto exit;
  }
  ...
  if (ret < 0)
  {
    printf("Failed to open pseudo tty.\n");
    goto exit;
  }
  ...
  {
    printf("Failed to allocate memory to stream\n");
    goto exit;
  }
