...
{
  unsigned char fcs = FCS_MAX_VALUE;
  int i;
  for (i = 0; i < count; i++)
    {
  ...
  while (cmux_buffer_length(cmux_buffer) >= CMUX_MIN_FRAME_LEN)
    {
      ...
      fcs = FCS_MAX_VALUE;
      cmux_parse->address = ((*data & ADDR_FIELD_CHECK) >> 2);
      ...
      cmux_parse->data_length = (*data & LENGTH_FIELD_OPERATOR) >> 1;
      fcs = g_cmux_crc_table[fcs ^ *data];
      ...
      if (g_cmux_crc_table[fcs ^ (*data)] != FCS_OPERATOR)
        {
                            ...
                            int frame_size, unsigned char type)
{
  ...
  unsigned char frame_prefix[CMUX_FRAME_PREFIX] = {
                            F_FLAG,
                            (ADDR_FIELD_BIT_EA | ADDR_FIELD_BIT_CR),
                            0x00,
                            0x00,
                            0x00
                          };
  ...
  ((ADDR_FIELD_OPERATOR & (unsigned char)channel) << 2));
  ...
  if (frame_size <= CMUX_FRAME_MAX_SIZE)
    ...
    {
      frame_prefix[CMUX_BIT3] = (frame_size << 1) & LENGTH_FIELD_OPERATOR;
...
static int cmux_extract(struct cmux_ctl_s *ctl, char *input, int len)
{
  ...
  while (cmux_decode_frame(ctl->stream, ctl->parse) >= 0)
    {
      if (CMUX_FRAME_TYPE(FRAME_TYPE_UI, ctl->parse) ||
          CMUX_FRAME_TYPE(FRAME_TYPE_UIH, ctl->parse))
        {
